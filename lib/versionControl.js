    ...
    async diff (commitOrCommitId, req) {
      ...
      return commitToDiff.changes.reduce((res, c) => {
        ...
        if (c.operation === 'remove') {
          ...
          patch = createPatch(c.path, previousEntity.entity, {}, c.entitySet, documentModel, diffLimit, { context: Number.MAX_VALUE, bufferEncoding: 'utf8' })
        }

        if (c.operation === 'insert') {
          patch = createPatch(c.path, {}, parse(c.serializedDoc), c.entitySet, documentModel, diffLimit, { context: Number.MAX_VALUE, bufferEncoding: 'utf8' })
        }

        if (c.operation === 'update') {
          const previousEntity = previousState.find((s) => s.entityId === c.entityId)
          const afterEntity = commitState.find((s) => s.entityId === c.entityId)
          patch = createPatch(c.path, previousEntity.entity, afterEntity.entity, c.entitySet, documentModel, diffLimit, { context: Number.MAX_VALUE, bufferEncoding: 'utf8' })
        }
    ...
    async commit (message, preview, req) {
      ...
      newVersion.changes = lastState.reduce((res, s) => {
        ...
        return res.concat({
          ...
          entitySet: s.entitySet,
          entityId: s.entityId,
          serializedPatch: serialize(createPatch(s.path, s.entity, entity, s.entitySet, documentModel, diffLimit))
        })
      }, [])
