    ...
    async diff (commitOrCommitId, req) {
      ...
      const diff = await Promise.reduce(commitToDiff.changes, async (res, c) => {
        ...
        if (c.operation === 'remove') {
          ...
          // support for old versions entities that were not storing the
          // full path in changes
          if (change.fullPath == null) {
            change.fullPath = await reporter.folders.getFullPath(previousEntity.entity, c.entitySet, req, (folderShortId) => {
              const found = previousState.find((ls) => ls.entitySet === 'folders' && ls.entity.shortid === folderShortId)
        ...
        if (c.operation === 'insert') {
          ...
          // full path in changes
          if (change.fullPath == null) {
            change.fullPath = await reporter.folders.getFullPath(doc, c.entitySet, req, (folderShortId) => {
              const found = commitState.find((ls) => ls.entitySet === 'folders' && ls.entity.shortid === folderShortId)
        ...
        if (c.operation === 'update') {
          ...
          // full path in changes
          if (change.fullPath == null) {
            change.fullPath = await reporter.folders.getFullPath(afterEntity.entity, c.entitySet, req, (folderShortId) => {
              const found = commitState.find((ls) => ls.entitySet === 'folders' && ls.entity.shortid === folderShortId)
        ...
        return res.concat({
          ...
          fullPath: change.fullPath + '/' + p.path.split('.').slice(-1)[0],
          patch: p.patch,
          type: p.type
        })))
      }, [])
      ...
      newVersion.changes = await Promise.reduce(lastState, async (res, s) => {
        ...
        if (!entity) {
          ...
          return res.concat({
            ...
            fullPath: await reporter.folders.getFullPath(s.entity, s.entitySet, req, (folderShortId) => {
        ...
        return res.concat({
          operation: 'update',
          path: entityPath(s.entity, s.entitySet),
          fullPath: await reporter.folders.getFullPath(entity, s.entitySet, req),
          entitySet: s.entitySet,
          entityId: s.entityId,
        ...
        return Promise.each(currentEntities[es], async (e) => {
          if (!lastState.find((s) => s.entityId === e._id && s.entitySet === es)) {
            newVersion.changes.push({
              ...
              fullPath: await reporter.folders.getFullPath(e, es, req),
              entitySet: es,
              entityId: e._id,
