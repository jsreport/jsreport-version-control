      ...
      newVersion.changes = await Promise.reduce(lastState, async (res, s) => {
        ...
        if (!entity) {
          ...
          return res.concat({
            ...
            path: await reporter.folders.getFullPath(s.entity, s.entitySet, req, (folderShortId) => {
        ...
        return res.concat({
          operation: 'update',
          path: await reporter.folders.getFullPath(entity, s.entitySet, req),
          entitySet: s.entitySet,
          entityId: s.entityId,
        ...
        return Promise.each(currentEntities[es], async (e) => {
          if (!lastState.find((s) => s.entityId === e._id && s.entitySet === es)) {
            newVersion.changes.push({
              operation: 'insert',
              path: await reporter.folders.getFullPath(e, es, req),
              entitySet: es,
              entityId: e._id,
