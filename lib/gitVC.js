...
module.exports = (reporter) => {
  ...
  async function diffToChanges (diff) {
    ...
    for (const patch of patches) {
      ...
      p.patch.config.push('Index: ' + patch.newFile().path())

      p.patch.config.push('===================================================================')
      p.patch.config.push('--- ' + patch.newFile().path())
      p.patch.config.push('+++ ' + patch.newFile().path())
      ...
      for (const hunk of hunks) {
        ...
        p.patch.config.push.apply(p.patch.config, lines.map((l) => String.fromCharCode(l.origin()) + l.content().trim()))
      }

      p.patch.config = p.patch.config.join('\n') + '\n'
    }
