...
module.exports.createPatch = (name, oldEntity, newEntity, entitySet, documentModel, diffLimit = 400 * 1024, options = { context: 0, bufferEncoding: 'base64' }) => {
  const patch = {
    documentProperties: []
  }
  ...
  documentModel.entitySets[entitySet].entityType.documentProperties.forEach((p) => {
    ...
    // files bigger than 500kb are not "diffed"
    if (Buffer.isBuffer(older) && (older.length > diffLimit || newer.length > diffLimit)) {
