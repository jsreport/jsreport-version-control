...
module.exports = (reporter, definition) => {
  const options = Object.assign(definition.options, reporter.options.versionControl)
  ...
  reporter.initializeListeners.add('version-control', () => {
    const provider = reporter.versionControl.providers[options.name || 'default']
    if (!provider) {
      throw new Error(`Version control provider with name ${options.name} not registered`)
    }
